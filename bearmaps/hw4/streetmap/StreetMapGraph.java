...
public class StreetMapGraph implements AStarGraph<Long> {
    ...
    /**
     * Returns the great-circle distance between S and GOAL. Assumes S and GOAL exist in this graph.
     * 返回S和GOAL之间的大圆距离。假设S和GOAL存在于这个图形中。
     */
    @Override
     ...
     * 工厂方法。从OSM XML文件中创建并返回一个图形。假设文件的格式正确。
     */
     ...
     *  如果FROMID和TOID都在这个图中，则向这个图添加一条边。如果边已经存在，则不添加额外的边。
     **/
     ...
     * 移除图中外度为0的顶点。注意，如果边不是双向的，这将导致问题。
     **/
    private void clean() {
     ...
     * 获取一个顶点的纬度。
     * @param v The id of the vertex.顶点的ID。
