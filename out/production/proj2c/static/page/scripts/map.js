...
$(function() {
    ...
    $('body').dblclick(function handler(event) {
        if (route_params.start_lon && route_params.end_lon) { //完成导航，重置导航
            route_params = {};
        }
        ...
        if (route_params.start_lon) { // began routing already but not finished已开始寻路，但未完成
    ...
    /* Enables scroll wheel zoom 启用滚轮缩放 */
    ...
    /* Prevent image dragging 防止图像拖动 */
    $('img').on('dragstart', function(event) { event.preventDefault(); });

    // Allow for window resizing 允许调整窗口的大小
